export const ADD_BOOK = 'ADD_BOOK';
export const ADD_BOOK_SUCCESS = 'ADD_BOOK_SUCCESS';
export const ADD_BOOK_ERROR = 'ADD_BOOK_ERROR';

export const DELETE_BOOK = 'DELETE_BOOK';
export const DELETE_BOOK_SUCCESS = 'DELETE_BOOK_SUCCESS';
export const DELETE_BOOK_ERROR = 'DELETE_BOOK_ERROR';

export const UPDATE_BOOK = 'UPDATE_BOOK';
export const UPDATE_BOOK_SUCCESS = 'UPDATE_BOOK_SUCCESS';
export const UPDATE_BOOK_ERROR = 'UPDATE_BOOK_ERROR';

export const FETCHING_BOOKS = 'FETCH_BOOKS';
export const FETCHING_BOOKS_SUCCESS = 'FETCHING_BOOKS_SUCCESS';
export const FETCHING_BOOKS_ERROR = 'FETCHING_BOOKS_ERROR';

export const SEARCH_BOOK = 'SEARCH_BOOK';
export const SEARCH_BOOKS_SUCCESS = 'SEARCH_BOOKS_SUCCESS';
export const SEARCH_BOOKS_ERROR = 'SEARCH_BOOKS_ERROR';

export const FETCHING_BOOKS_TYPE = 'FETCHING_BOOKS_TYPE';
export const FETCHING_BOOKS_TYPE_SUCCESS = 'FETCHING_BOOKS_TYPE_SUCCESS';
export const FETCHING_BOOKS_TYPE_ERROR = 'FETCHING_BOOKS_TYPE_ERROR';